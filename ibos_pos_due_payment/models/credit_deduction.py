    ...
    def action_generate_credit_customer(self):
        ...
        for partner in partners:
            if partner:
                ...
                # deduct amount
                deduct_amount = 0
                deduct_amount_ids = self.env['customer.credit.payment.line'].search([('partner_id', '=', partner.id), ('payment_date', '>=', start_date), ('payment_date', '<=', end_date)])
                for deduct_amount_id in deduct_amount_ids:
                    deduct_amount += deduct_amount_id.amount

                # actual credit after except deduction amount
                credit = credit - deduct_amount
